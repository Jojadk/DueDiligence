<?php
/**
 * DueDiligence v2.0 - SVG Icons
 *
 * Icon system for the application
 */
...
function icon(string $name, int $size = 24, string $class = ''): string {
    $icons = [
        ...
        'settings' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"></circle><path d="M12 1v6m0 6v6m5.5-13.5-5.5 5.5-5.5-5.5m11 11-5.5-5.5-5.5 5.5"></path></svg>',
        ...
        'archive' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="21 8 21 21 3 21 3 8"></polyline><rect x="1" y="3" width="22" height="5"></rect><line x1="10" y1="12" x2="14" y2="12"></line></svg>',
    ...
    $svg = $icons[$name] ?? $icons['file'];

    // Add size and class attributes
    $svg = str_replace('<svg', '<svg width="' . $size . '" height="' . $size . '" class="icon icon-' . $name . ($class ? ' ' . $class : '') . '"', $svg);
 ...
 * Echo icon (shorthand)
 */
function echo_icon(string $name, int $size = 24, string $class = ''): void {
    echo icon($name, $size, $class);
}
